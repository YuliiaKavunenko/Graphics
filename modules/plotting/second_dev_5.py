...
def fifth_second_dev():
    check = second_dev_fifth.get()  # Отримання значення чекбоксу / Getting the value of the checkbox
    plot_fifth_second = dictionary_of_variables['plot_fifth_second']  # Отримання графіку зі словника / Getting the plot from the dictionary
    ox_points_second_5 = dictionary_of_variables['ox_points_second_5']  # Отримання точок перетину з віссю x / Getting the intersection points with the x-axis
    h_lines_second_5 = dictionary_of_variables['h_lines_second_5']  # Отримання пунктирних ліній / Getting the dashed lines
    inflection_points_scatter_5 = dictionary_of_variables['inflection_points_scatter_5']  # Отримання точок перегину / Getting the inflection points
    ...
    if check == 1:  # Якщо чекбокс активований / If the checkbox is checked
        ...
        if a1 and a2:  # Перевірка, чи існують значення a і b / Checking if a and b values exist
            ...
            if inflection_points:
                if sympy.im(point) == 0:  # Тільки дійсні точки
                    ...
                    inflection_points_label_5.configure(text=f"9) Точки перегину: {formatted_points}")  # Оновлення тексту мітки з точками перегину / Updating the text for the inflection points label
            else:
                inflection_points_label_5.configure(text="9) Точки перегину: не існує")  # Виведення повідомлення про відсутність точок перегину / Displaying message about the absence of inflection points
            ...
            ox_points_second_5 = points_0x_0y['0x']  # Точки 0х / Points on the x-axis
            h_lines_second_5 = points_0x_0y['lines']  # Пунктирні лінії / Dashed lines
            ...
            dictionary_of_variables['ox_points_second_5'] = ox_points_second_5  # Збереження точок перетину з віссю x / Storing intersection points with the x-axis
            dictionary_of_variables['h_lines_second_5'] = h_lines_second_5  # Збереження пунктирних ліній / Storing dashed lines
            dictionary_of_variables['plot_fifth_second'] = plot_fifth_second  # Збереження графіку другої похідної / Storing the second derivative graph
            dictionary_of_variables['inflection_points_scatter_5'] = inflection_points_scatter_5  # Збереження точок перегину / Storing inflection points
            dictionary_of_variables['inflection_points_label_5'] = inflection_points_label_5  # Збереження лейблу з точками перегину / Storing the label with inflection points
        ...
        if ox_points_second_5:
            ...
            ox_points_second_5.clear()

        if h_lines_second_5:
            for line in h_lines_second_5:
                line.remove()  # Видалення горизонтальних ліній / Removing horizontal lines
            h_lines_second_5.clear()

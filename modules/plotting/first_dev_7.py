...
def seventh_first_dev():
    ...
    # Перевірка, чи увімкнено четверту похідну / Check if theseventh derivative is enabled
    check = first_dev_seventh.get()

    plot_seventh_first = dictionary_of_variables['plot_seventh_first']
    ...
    if check == 1:  # Якщо чекбокс ввімкнено / If the checkbox is enabled
        ...
        # Отримання значення змінної "a" для четвертої похідної / Retrieve the value of variable "a" for theseventh derivative
        a1 = a1_seventh.get()

        if a1:  # Перевірка, чи значення "a" не є порожнім / Check if "a" value is not empty
            ...
            function = (x**2 + x + a1)/x # Основна функція / The main function
            ...
            if isinstance(dev_of_function, sympy.Number):  # Якщо вираз є числом / If the expression is a number
                ...
                if intervals_data['локальний максимум'] != "не існує":  # Перевірка наявності локальних максимумів / Check for local maxima
                    ...
                    local_max_seventh_first = ax.scatter(l_max_x, l_max_y, color='#FF0899', s=40)  # Додавання точок локального максимуму / Adding local maximum points
                ...
                if intervals_data['локальний мінімум'] != "не існує":  # Перевірка наявності локальних мінімумів / Check for local minima
                    ...
                    local_min_seventh_first = ax.scatter(l_min_x, l_min_y, color='#FF0899', s=40)  # Додавання точок локального мінімуму / Adding local minimum points
                ...
                drob_first_dev_lable.configure(text=f"y' = {seventh_f_dev_label}")
                ...
                plot_seventh_first = ax.plot(x_vals, y_vals, label=f"y' = {dev_of_function}", color='green')
                ...
                ox_points_seventh_first = points_0x_0y['0x']  # Ось ми присвоюємо координати X точок до змінної / Here we assign the X coordinates of the points to a variable
                h_lines_seventh_first = points_0x_0y['lines']  # Тут ми присвоюємо лінії до іншої змінної / Here we assign lines to another variable
                ...
                dictionary_of_variables['plot_seventh_first'] = plot_seventh_first
    ...
    # Перевірка стану check і наявності графіка / Checking the state of 'check' and presence of graph
        ...
        if plot_seventh_first:
            for line in plot_seventh_first:
                line.remove()
        ...
        if ox_points_seventh_first:
            for point in ox_points_seventh_first:
                point.remove()  # Видалення кожної точки / Removing each point
            ox_points_seventh_first.clear()  # Очищення списку точок / Clearing the list of points

        if h_lines_seventh_first:

        ...
        if a1 and a2:  # Перевірка, чи значення "a" не є порожнім / Check if "a" value is not empty
            ...
            if isinstance(dev_of_function, sympy.Number):  # Якщо вираз є числом / If the expression is a number
                ...
                if intervals_data['локальний максимум'] != "не існує":
                    ...
                    hover_points.append(local_max_fifth_first)
                    local_max_text_fifth_first = ax.annotate(f'({l_max_x:.2f}, {l_max_y:.2f})',
                                                            ...
                                                            ha='center',
                                                            visible=False)
                    hover_annotations.append(local_max_text_fifth_first)
                else:
                ...
                if intervals_data['локальний мінімум'] != "не існує":
                    ...
                    hover_points.append(local_min_fifth_first)
                    local_min_text_fifth_first = ax.annotate(f'({l_min_x:.2f}, {l_min_y:.2f})',
                                                            ...
                                                            ha='center',
                                                            visible=False)
                    hover_annotations.append(local_min_text_fifth_first)
                else:
                ...
                wrapped_label = '\n'.join(wrap(label, 60))
                plot_fifth_first = ax.plot(x_vals, y_vals, label=f"y' = {wrapped_label}", color='green')

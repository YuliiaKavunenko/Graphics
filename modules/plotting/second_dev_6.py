...
def sixth_second_dev():
    ...
    if check == 1:  # Якщо чекбокс активований / If the checkbox is checked
        ...
        if a1 and a2:  # Перевірка, чи існують значення a і b / Checking if a and b values exist
            ...
            sixth_s_dev_label.configure(text=f"y'' = {rounded_derivative}")  # Оновлення тексту для другої похідної / Updating the text for the second derivative
            ...
            if isinstance(func, sympy.Number):  # Якщо вираз є числом / If the expression is a number
                ...
                for point in inflection_points:
                    if sympy.im(point) == 0:
                        ...
                        inflection_points_label_6.append(annotation)
                def on_hover(event):
                    if event.inaxes == ax:
        ...
        for label in inflection_points_label_6:
            label.remove()  # Видалення кожного підпису / Removing each label
        inflection_points_label_6.clear()

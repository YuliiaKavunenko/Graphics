...
def seventh_second_dev():
    ...
    if check == 1:  # Якщо чекбокс активований / If the checkbox is checked
        ...
        if a1:  # Перевірка, чи існують значення a і b / Checking if a and b values exist
            ...
            seventh_s_dev_label.configure(text=f"y'' = {second_dev_of_function}")  # Оновлення тексту для другої похідної / Updating the text for the second derivative
            ...
            if isinstance(func, sympy.Number):  # Якщо вираз є числом / If the expression is a number
                ...
                for point in inflection_points:
                    if sympy.im(point) == 0:  # Тільки дійсні точки
                        ...
                        inflection_label_7 = ax.annotate(f'({float(point):.2f}, {float(y_val):.2f})',
                                    (float(point), float(y_val)),
                                    textcoords="offset points",
                                    xytext=(0, 10),
                                    ha='center', color='blue')
                        inflection_points_label_7.append(inflection_label_7)
                ...
                plot_seventh_second = ax.plot(x_vals, y_vals, label=f"y'' = {second_dev_of_function}", color='blue')

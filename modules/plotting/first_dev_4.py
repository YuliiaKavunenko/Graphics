        ...
        if a:  # Перевірка, чи значення "a" не є порожнім / Check if "a" value is not empty
            ...
            if isinstance(dev_of_function, sympy.Number):  # Якщо вираз є числом / If the expression is a number
                ...
                if intervals_data['локальний максимум'] != "не існує":  # Перевірка наявності локальних максимумів / Check for local maxima
                    ...
                    local_max_fourth_first = ax.scatter(l_max_x, l_max_y, color='#FF0899', s=40)  # Додавання точок локального максимуму / Adding local maximum points
                ...
                if intervals_data['локальний мінімум'] != "не існує":  # Перевірка наявності локальних мінімумів / Check for local minima
                    ...
                    local_min_fourth_first = ax.scatter(l_min_x, l_min_y, color='#FF0899', s=40)  # Додавання точок локального мінімуму / Adding local minimum points

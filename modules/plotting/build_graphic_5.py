        ...
        try:
            ...
            if a1 != a2:
                ...
                if a1 != 0:
                    ...
                    ax.scatter(0, 1, facecolors='none', edgecolors='black', s = 50)  # Кружечок без заливки / Circle without fill
                    ax.annotate(f'({0}, {1})',
                                (0, 1),
                    ...
                    # Перевірка парності або непарності функції / Checking the function for evenness or oddness
                    even_or_odd_func_l.configure(text='2) Функція загального вигляду')  # Налаштування тексту для результату перевірки / Setting the text for the check result

                    intervals_identity_l.configure(text=f"8) Проміжки знакосталості:\n y > 0 при х ∈ (-∞; ∞)")  # Налаштування тексту мітки для проміжків знакосталості / Setting the text for the sign constancy intervals label
